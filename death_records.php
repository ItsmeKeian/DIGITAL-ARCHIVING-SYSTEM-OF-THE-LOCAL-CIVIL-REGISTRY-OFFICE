<?php
/* ============================================
   LCR DIGITAL ARCHIVING SYSTEM - DEATH RECORDS
...
$sql    = "SELECT d.*, u.full_name as added_by 
...
if (!empty($search)) {
    $sql .= " AND (d.deceased_first_name LIKE ? 
               OR d.deceased_last_name  LIKE ?
               OR d.registry_number     LIKE ?
               OR d.place_of_death      LIKE ?
               OR d.cause_of_death      LIKE ?)";
    $s = '%' . $search . '%';
    $params = array_merge($params, [$s, $s, $s, $s, $s]);
}

if (!empty($filter_year)) {
    $sql .= " AND YEAR(d.date_of_death) = ?";
    $params[] = $filter_year;
}
...
$yr_stmt = $pdo->query("SELECT DISTINCT YEAR(date_of_death) as yr FROM death_records ORDER BY yr DESC");
...
<head>
    ...
    <title>LCR Digital Archiving System | Death Records</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        <ul>
            ...
            <li><a href="death_records.php" class="nav-link active" data-tooltip="Death Records"><i class="fas fa-cross nav-icon"></i><span class="nav-text">Death Records</span></a></li>
...
<div class="main-wrapper" id="mainWrapper">
    <header class="topbar">
        ...
        <div class="topbar-title">Death Records</div>
    ...
    <main class="page-content">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
            <div>
                <h2 style="font-family:'Cinzel',serif;font-size:1.1rem;color:var(--primary);margin-bottom:3px;">Death Records</h2>
            ...
            <button class="btn btn-primary" id="btnAddRecord">
                <i class="fas fa-plus"></i> Add Death Record
            </button>
        </div>
        ...
        <div class="search-bar">
            <div class="search-input-wrap">
                ...
                <input type="text" class="search-input" id="searchInput" placeholder="Search by name, registry no., cause..." value="<?= htmlspecialchars($search) ?>"/>
            </div>
            ...
            <a href="death_records.php" class="btn btn-sm" style="background:#f0f0f0;color:#555;">
            ...
            <div style="overflow-x:auto;">
                <table class="lcr-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="10" style="text-align:center;color:#aaa;padding:32px;">
                        ...
                        <tr>
                            ...
                            <td><?= htmlspecialchars($rec['deceased_first_name'] . ' ' . $rec['deceased_middle_name'] . ' ' . $rec['deceased_last_name']) ?></td>
                            <td>
                                <span class="badge <?= $rec['deceased_sex'] === 'Male' ? 'badge-birth' : '' ?>"
                                      style="<?= $rec['deceased_sex'] === 'Female' ? 'background:rgba(106,27,154,0.1);color:#6A1B9A;' : '' ?>">
                                    <?= $rec['deceased_sex'] ?>
                                </span>
                            </td>
                            <td><?= date('M d, Y', strtotime($rec['date_of_death'])) ?></td>
                            <td><?= htmlspecialchars($rec['place_of_death']) ?></td>
                            <td><?= htmlspecialchars($rec['cause_of_death'] ?: '—') ?></td>
...
<div class="modal-overlay" id="recordModal">
    <div class="modal">
        <div class="modal-header">
            <div class="modal-title" id="modalTitle"><i class="fas fa-cross"></i> Add Death Record</div>
            ...
            <form id="recordForm" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="registry_number" id="registry_number" class="form-control" placeholder="e.g. 2024-D-001" required/>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="deceased_first_name" id="deceased_first_name" class="form-control" placeholder="First name" required/>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Middle Name</label>
                        <input type="text" name="deceased_middle_name" id="deceased_middle_name" class="form-control" placeholder="Middle name"/>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Last Name <span style="color:red">*</span></label>
                        <input type="text" name="deceased_last_name" id="deceased_last_name" class="form-control" placeholder="Last name" required/>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Sex <span style="color:red">*</span></label>
                        <select name="deceased_sex" id="deceased_sex" class="form-control" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="place_of_death" id="place_of_death" class="form-control" placeholder="e.g. San Julian District Hospital" required/>
                    </div>
                    <div class="form-group col-span-2">
                        <label class="form-label">Cause of Death</label>
                        <input type="text" name="cause_of_death" id="cause_of_death" class="form-control" placeholder="e.g. Cardiac Arrest"/>
                    </div>
...
<div class="modal-overlay" id="viewModal">
    <div class="modal">
        <div class="modal-header">
            <div class="modal-title"><i class="fas fa-eye"></i> View Death Record</div>
...
$('#btnAddRecord').on('click', function () {
    ...
    $('#record_id').val('');
    $('#formAction').val('add');
    $('#modalTitle').html('<i class="fas fa-cross"></i> Add Death Record');
    clearFile();
...
function viewRecord(id) {
    ...
    openModal('viewModal');
    $.ajax({
        url: 'php/death_handler.php?action=get&id=' + id,
        type: 'GET', dataType: 'json',
        success: function (res) {
            if (res.status === 'success') {
                const r = res.data;
                const fullName = [r.deceased_first_name, r.deceased_middle_name, r.deceased_last_name].filter(Boolean).join(' ');
                ...
                $('#viewModalBody').html(`
                    <div class="view-grid">
                        ...
                        <div class="view-field" style="grid-column:span 2"><div class="view-label">Deceased Full Name</div><div class="view-value" style="font-weight:700;font-size:1rem;">${fullName}</div></div>
                        <div class="view-field"><div class="view-label">Sex</div><div class="view-value">${r.deceased_sex}</div></div>
                        <div class="view-field"><div class="view-label">Date of Death</div><div class="view-value">${formatDate(r.date_of_death)}</div></div>
                        <div class="view-field"><div class="view-label">Place of Death</div><div class="view-value">${r.place_of_death}</div></div>
                        <div class="view-field"><div class="view-label">Cause of Death</div><div class="view-value">${r.cause_of_death || '—'}</div></div>
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status === 'success') {
                ...
                $('#record_id').val(r.id);
                $('#formAction').val('edit');
                $('#modalTitle').html('<i class="fas fa-pen"></i> Edit Death Record');
                $('#registry_number').val(r.registry_number);
                $('#date_registered').val(r.date_registered);
                $('#deceased_first_name').val(r.deceased_first_name);
                $('#deceased_middle_name').val(r.deceased_middle_name);
                $('#deceased_last_name').val(r.deceased_last_name);
                $('#deceased_sex').val(r.deceased_sex);
                $('#date_of_death').val(r.date_of_death);
                $('#place_of_death').val(r.place_of_death);
                $('#cause_of_death').val(r.cause_of_death);
